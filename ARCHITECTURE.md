...
**Chọn issue theo bằng chứng (không tin claim):** claim của khách không phải ground truth nên Coordinator lấy đủ mọi domain mà các issue phụ thuộc (`EXAMINE_ALL`: order, items, payment timeline, refund timeline, shipment, cộng `get_policy`; khoảng 6 call mỗi case). Khi nhiều issue cùng được evidence hỗ trợ, `anchored_issues` ([rules.py](src/student_agent/rules.py)) giữ lại issue có tín hiệu nằm trên dòng thời gian của chính đơn: khoản thanh toán/sai lệch/trùng phí được ghi nhận vào ngày mua, yêu cầu hoàn tiền rơi trong 7 ngày sau ngày giao. Tín hiệu gây nhiễu (từ đơn khác) mang ngày không liên quan nên bị loại. Nếu claim nằm trong tập issue khớp dòng thời gian thì chọn claim; nếu tín hiệu của claim lệch dòng thời gian mà issue khác thì khớp, chọn issue khớp với confidence 0.75. Nếu chỉ có bằng chứng một phần (`full=False`, ví dụ khi replan) mà claim chưa được xác nhận thì Policy phát `widen_evidence` và Verifier yêu cầu mở rộng sang `FULL_PLAN` trước khi kết luận.
